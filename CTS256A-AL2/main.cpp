...
void help()
{
	puts(
		...
		" -xFile    Optional exception ROM image filename\n"
		...
		" -d        Debug mode\n"
		" -n        Suppress 'O.K.'\n"
		" -aAddr    Start address (in hex) of exception ROM\n"
...
static bool read_rom_file(const char *filename, std::vector<uchar>& rom)
{
	...
	if ( !rom_file.read( reinterpret_cast<char*>( rom.data() ), size) )
	{
		return false;
	}
...
int main(int argc, char* argv[])
{
	...
	std::vector<uchar> exception_rom{};
	ushort rom_address;
	...
	for ( int i=1; i<argc; ++i )
	{
		...
		if ( opts && *s == '-' )
		{
			...
			case 'I':
				...
				noOK = true;
				break;
			case 'X': // Exception ROM file
				++s;
				if ( *s == ':' )
					s++;
				if ( !read_rom_file( s, exception_rom ) )
				{
					console.printf( "Failed to open %s\n", s );
					return 1;
				}
			...
			case 'A':
				...
				{
     				rom_address =
						static_cast<ushort>(std::stoul( s, nullptr, 16 ));

					if ( ( rom_address & 0xFFF ) != 0 ||
				         ( rom_address < 0x1000 ) || ( rom_address > 0xE000 ) )
					{
						console.printf( "Invalid EPROM address.");
						return 1;
					}
				}
    			catch (const std::exception& e)
				{
					console.printf( "Unrecognized address: %s\n", s );
					return 1;
			    }
				break;
			case 'B': // Bin file
	...
	CTS256A_AL2 system( *pistr, *postr, std::move( exception_rom ), rom_address );
